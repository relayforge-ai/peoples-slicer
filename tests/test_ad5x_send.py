...
def _multicolor_detail():
    return {"status": "ready", "matlStationInfo": {"slotInfos": [
...
def test_multicolor_send_routes_to_printGcode_with_ifs_map(tmp_path, monkeypatch):
    ...
    a = AD5XAdapter(
        "h", "s", "c",
        detail_fetcher=_multicolor_detail,
...
def test_plain_multicolor_gcode_routes_to_printGcode_with_ifs_map(tmp_path, monkeypatch):
    fake = FakeSock()
    _patch_socket(monkeypatch, fake)
    gc = tmp_path / "duo.gcode"
    gc.write_text(
    ...
    assert adapter.tool_colors(str(gc)) == ["#FF0000", "#00FF00"]
    adapter.send(str(gc), start=True)

    sent = b"".join(fake.sent)
    assert b"M23" not in sent
    assert b"M6030" not in sent
    assert posted[-1][0] == "/printGcode"
    assert posted[-1][1]["useMatlStation"] is True
    assert posted[-1][1]["gcodeToolCnt"] == 2
